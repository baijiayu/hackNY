...
def convert_num_to_rgb (n):
    # n should be a float
    # n should be between -9999 and 9999
    m = (n+9999) * 10
    r = int(m // 10**4)
    g = int(m // 10**2 - r * 100)
    b = int(m % 100)
...
def row_function(A,x,y):
    ...
    (r1,g1,b1) = S[x // dx][y // dy]
    # (30, 56) --> (3,1)
    temp = y//dy + 1
    if(temp >= width):
        temp = width - 2
    print(x//dx,temp)
    (r2,g2,b2) = S[x // dx][temp]
    # (30, 56) --> (3,2)
...
def get_pixel(A,x,y):
    ...
    if (x % dx == 0): # x is on the main-row:
        ...
        (K,B) = find_first(x0,r0,x1,r1)

        final_r = K * x + B
        final_g = math.log(final_r)
        final_b = (final_r + final_g)/2
